#!/bin/bash
#---------------------------------------------------------------------
# Script Name:                          ambest_failed_notification.sh
...
if [ -f $$ambest_jobid.txt ] ; then

    rm $ambest_jobid.txt.txt
fi


####################Reading the ambest failed job#########################

/bin/psql -h $PG_GCMP_SERVER_NAME -d $PG_GCMP_DB -U $PG_GCMP_DB_USER_ID -p $PG_GCMP_PORT -c "\copy (
  select max(job_id) as job_id from iics.di_job_metadata where lower(job_status)='inprogress'  and  application_name in (select application_name from iics.rating_type where ratingtype_Id=1)  
)  to '$RATING_PATH/ambest_jobid.txt' DELIMITER '|'  " &> $RATING_PATH/ambest_jobid.log;
...
done <$RATING_PATH/ambest_jobid.txt


######## Notifying E-mail about the S&P failed job ##################  
echo "AMBest ratings feed dataload failed(Job_ID:$JOBID ) at:`date`" | mailx -r "${Mail_Sender}" -s "AMBest ratings feed dataload failed in  ${IICS_SERVER}" "${RECIPIENTS}"